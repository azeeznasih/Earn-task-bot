...
app.get("/receipt/:id", async (req, res) => {
  try {
    ...
    <style>
      ...
      .provider-name{color:#00ffcc;font-weight:700;text-decoration:none;border-bottom:1px dashed #00ffcc;padding-bottom:2px;cursor:pointer;}
      .provider-name:active{opacity:0.7;}
    </style></head><body>
    <div class="container">
...
async function approveUPIPayment(orderId, utr) {
...
app.get("/miniapp/api/user/:userId", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      user: {
        userId: user.userId, firstName: user.firstName, username: user.username,
        balance: user.balance, withdrawnTotal: user.withdrawnTotal,
        blockedRefs: user.blockedRefs, referredBy: user.referredBy,
        joined: user.createdAt, linkedInfo,
        upiId: user.upiId, bankAccNo: user.bankAccNo, bankIfsc: user.bankIfsc,
        amazonEmail: user.amazonEmail, redeemCodeAddr: user.redeemCodeAddr,
...
// ============================================================
// 💠 UPI VERIFY API (Auto Add Balance)
// ============================================================
app.post("/miniapp/api/upi/verify", async (req, res) => {
  try {
    ...
    const existing = await UPIPayment.findOne({ utr: cleanUtr, status: "Approved" });
    if (existing) {
      return res.json({ success: false, error: "UTR already used" });
    }

    const orderId = `ORD${Date.now()}${Math.floor(Math.random() * 1000)}`;
    const autoVerify = await getConfig("auto_upi_verify", true);

    if (autoVerify) {
      await UPIPayment.create({
        orderId, userId: uid, amount: amt,
        utr: cleanUtr, upiId: upiId || "nasih@fam",
        status: "Approved", source: "miniapp",
        verifiedAt: new Date()
      });
      ...
      await logBalanceHistory(uid, `UPI Deposit (UTR: ${cleanUtr})`, amt);

      const payoutChannel = await getConfig("payout_channel", null);
      if (payoutChannel) {
        try {
          await bot.api.sendMessage(payoutChannel,
            `💰 <b>New UPI Deposit (MiniApp)!</b>\n\n` +
      ...
      if (payoutChannel) {
        ...
        try {
          await bot.api.sendMessage(payoutChannel,
            `💰 <b>UPI Deposit Request (MiniApp)</b>\n\n` +
            `<b>👤 User:</b> ${user.firstName || "User"}\n` +
            `<b>🆔 ID:</b> <code>${uid}</code>\n` +
            `<b>💵 Amount:</b> ₹${amt}\n` +
            `<b>🔐 UTR:</b> <code>${cleanUtr}</code>\n` +
...
// 📝 PART 1 END
// ============================================================
// Part 2-ൽ തുടരും: Bot commands, Add Fund flow, Admin panel
// ============================================================
  ...
  if (state) {
    ...
    if (state.startsWith("AFM_EDIT_MAX_") && (await isAdmin(userId))) {
      let key = state.replace("AFM_EDIT_MAX_", "");
              `<b>✅ Status:</b> Auto-Approved\n` +
              `<b>📅 Date:</b> ${new Date().toLocaleString(      delete userState[userId];
      let amt = parseFloat(text);
      if (isNaN(amt) || amt < 1) return ctx.reply("❌ Invalid!");
...
'en-IN')}`,
    ...
    if (state === "WAITING_GW_NAME" && (await isAdmin(userId))) {
      ...
      return ctx.reply(`🌐 Gateway: ${gwName}\n\n📝 Send Gateway URL:`, { parse_mode: "Markdown", reply_markup: new InlineKeyboard().text("🔙 Cancel", "adm_gateway_menu") });
    }
    ...
    if (state === "WAITING_FOR_ADD_BAL" && (await isAdmin(userId))) {
      ...
      if (!targetUser) {
        targetUser = await User.create({
          userId: targetId, firstName: "Unknown", balance: amount,
    ...
    if (state === "SET_AMAZON_ACC") {
      ...
      return ctx.reply(`✅ Email Updated!`, { parse_mode: "Markdown", reply_markup: await buildKeyboardFromLayout() });
    }
    if (state === "SET_REDEEM_ACC") {
      delete userState[userId];
      await User.findOneAndUpdate({ userId }, { redeemCodeAddr: text.trim() });
      return ctx.reply(`✅ Redeem Code Updated!`, { parse_mode: "Markdown", reply_markup: await buildKeyboardFromLayout() });
    }
...
bot.callbackQuery("adm_customize_theme", async (ctx) => {
  ...
  await ctx.editMessageText("⚙️ *Manage Keyboard:*\n\nReply keyboard layout-ൽ മാറ്റം വരുത്താൻ /admin → Theme use ചെയ്യൂ.", {
    parse_mode: "Markdown",
...
bot.callbackQuery("broadcast_confirm", async (ctx) => {
  ...
  await ctx.editMessageText(
    ...
    {
      parse_mode: "Markdown",
      reply_markup: new InlineKeyboard().text("🗑️ Remove", `broadcast_remove_${bcId}`).row().text("🔙 Back", "admin")
    }
  ).catch(() => {});
});
...
bot.callbackQuery(/^upi_app_/, async (ctx) => {
  ...
  let result = await approveUPIPayment(orderId, payment.utr || "");
...
bot.callbackQuery("adm_toggle_bot", async (ctx) => {
  ...
  return bot.callbackQuery("adm_settings", ctx);
});
...
bot.callbackQuery(/^wt_toggle_/, async (ctx) => {
  ...
  return bot.callbackQuery("adm_withdraw_toggle", ctx);
});
...
bot.callbackQuery(/^wd_app_/, async (ctx) => {
  ...
  await ctx.editMessageText(updatedMsg, { parse_mode: "HTML" }).catch(() => {});

  let accountType = wd.method === "UPI" ? "UPI" : (wd.method === "Wallet" ? "Wallet" : (wd.method === "Bank" ? "Bank" : "Redeem"));
  let dateStr = formatDateTime(wd.approvedAt);
...
// 🎉 END OF FILE
// ============================================================
console.log("✅ index.js loaded — Bot + Server + All APIs");
